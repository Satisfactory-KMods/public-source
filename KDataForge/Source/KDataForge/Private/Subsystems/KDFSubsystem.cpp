...
void UKDFSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	RegisterHandler(NewObject<UKDFSublevelHandler>(this));
	...
	RegisterHandler(NewObject<UKDFDataAssetFinalizeHandler>(this));
	...
	RegisterHandler(NewObject<UKDFResourceNodeHandler>(this));
...
void UKDFSubsystem::Deinitialize()
{
	...
	FKDFValueCodec::SetDynamicContentRegistry(nullptr);
	mActorPatches.Reset();
...
void UKDFSubsystem::ScanAndParse()
{
	...
	mPacks.RemoveAll(
		...
		{
			FString FailReason;
			if (FKDFConditionEvaluator::Evaluate(Pack.mConditionsNode.Get(), Pack.mConditionBehaviorNode.Get(),
											 ConditionsGameInstance, FailReason))
			{
				return false;
			}
			FKDFDiagnostic& Diagnostic = mDiagnostics.AddDefaulted_GetRef();
	...
	for (int32 Index = 0; Index < PendingFiles.Num(); ++Index)
	{
		...
		for (int32 DocIndex = 0; DocIndex < Result.mRoots.Num(); ++DocIndex)
		{
			...
			Document.mRootType = DetectRootType(*DocRoot, Pending.mRelativeFile,  !bMultiDoc);
			...
			for (const FString& Include : Document.mIncludeDependencies)
			{
				for (int32 Later = Position + 1; Later < DocumentIndices.Num(); ++Later)
				{
						...
						DocumentMatchesInclude(Candidate.mRelativeFile, Include))
					{
						...
						DocumentIndices.Insert(Moved, Later);
						bChanged = true;
						break;
					}
				}
...
void UKDFSubsystem::ApplyStages()
{
	...
	{
		const FKDFNode* ConditionBehaviorNode = mDocuments[Index].mRoot->Find(TEXT("conditionBehaivor"));
		if (ConditionBehaviorNode == nullptr)
		{
			ConditionBehaviorNode = mDocuments[Index].mRoot->Find(TEXT("conditionBehavior"));
		}
		FString FailReason;
		ConditionsPassed[Index] = FKDFConditionEvaluator::Evaluate(mDocuments[Index].mRoot->Find(TEXT("conditions")),
																   ConditionBehaviorNode, GameInstance, FailReason);
...
{
	if (World == nullptr ||
		(mContentRegistrations.IsEmpty() && mSinkTableRequests.IsEmpty() && mContentRemovals.IsEmpty()))
	{
		return;
	}
	...
	for (const FKDFContentRemoval& Removal : mContentRemovals)
	{
		UClass* ContentClass = Removal.mContentClass.Get();
		if (!IsValid(ContentClass))
		{
			continue;
		}
		...
		{
		case EKDFContentRegKind::Schematic:
			ContentRegistry->RemoveSchematic(ContentClass);
			break;
		case EKDFContentRegKind::ResearchTree:
			ContentRegistry->RemoveResearchTree(ContentClass);
			break;
		default:
	...
	UE_LOG(LogKDataForge, Log,
		   TEXT("Registered %d queued content entr%s into the world's content registry (%d removal(s))"),
		   RegisteredCount, RegisteredCount == 1 ? TEXT("y") : TEXT("ies"), RemovedCount);
}
...
void UKDFSubsystem::RegisterNodePurge(const FKDFNodePurge& Purge)
{
	mNodePurges.RemoveAll(
		[&Purge](const FKDFNodePurge& Existing)
		{ return Existing.mResourceClass == Purge.mResourceClass && Existing.mSourceFile == Purge.mSourceFile; });
	mNodePurges.Add(Purge);
}

void UKDFSubsystem::RegisterContentRemoval(const FKDFContentRemoval& Removal)
{
	mContentRemovals.RemoveAll(
		[&Removal](const FKDFContentRemoval& Existing)
		{ return Existing.mContentClass == Removal.mContentClass && Existing.mSourceFile == Removal.mSourceFile; });
	mContentRemovals.Add(Removal);
}
...
void UKDFSubsystem::NotifyUObjectCreated(const UObjectBase* Object, int32  )
{
...
bool UKDFSubsystem::ProcessPendingLazyClasses(float  )
{
	check(IsInGameThread());
	if (!bLoadedOnce)
	{
...
void UKDFSubsystem::ApplyLazyWatchesToClass(UClass* NewClass, UObject* CDO)
{
	...
	const bool bMapOwnedClass =
		Package && Package->HasAnyPackageFlags(PKG_ContainsMap | PKG_ContainsMapData);
