...
namespace
{
	...
	bool ApplyBitmaskOp(EKDFOp Op, const FNumericProperty* Numeric, const UEnum* Enum, void* ValuePtr,
						const FKDFOpArgs& Args, FString& OutError)
	{
		switch (Op)
		{
		case EKDFOp::Clear:
			Numeric->SetIntPropertyValue(ValuePtr, int64(0));
			return true;
		case EKDFOp::Append:
		case EKDFOp::Prepend:
			{
				...
				{
					OutError = TEXT("Missing 'value' for append");
					return false;
				}
				int64 Value = 0;
				if (!FKDFValueCodec::ResolveBitmaskFlags(*Args.mValue, Enum, Value, OutError))
				{
					return false;
				}
				const int64 Current = Numeric->GetSignedIntPropertyValue(ValuePtr);
				Numeric->SetIntPropertyValue(ValuePtr, Current | Value);
				return true;
			}
		case EKDFOp::Remove:
			{
				if (Args.mValue == nullptr)
				{
					OutError = TEXT("Missing 'value' for remove");
					return false;
				}
				int64 Value = 0;
				if (!FKDFValueCodec::ResolveBitmaskFlags(*Args.mValue, Enum, Value, OutError))
		...
		default:
			OutError = FString::Printf(TEXT("Op '%s' is not supported on bitmask properties"),
									   *FKDFOpEngine::OpToString(Op));
			return false;
		}
	}
	...
	const FObjectProperty* GetInstancedInner(const FProperty* Inner)
	{
		return CastField<FClassProperty>(Inner) != nullptr ? nullptr : CastField<FObjectProperty>(Inner);
	}

	UObject* MakeInstancedObjectFromNode(UObject* Outer, const FObjectProperty* Inner, const FKDFNode& Entry,
										 FString& OutError)
	{
		if (!Entry.IsMap())
		{
			OutError = TEXT("Instanced object entry must be a map with a 'class' key");
			return nullptr;
		}
		const FString ClassPath = Entry.GetString(TEXT("class"), FString());
		if (ClassPath.IsEmpty())
		{
			OutError = TEXT("Instanced object entry needs a 'class'");
			return nullptr;
		}
		UClass* Class = FKDFValueCodec::ResolveClass(ClassPath, OutError);
		if (Class == nullptr)
		{
			return nullptr;
		}
		if (Inner->PropertyClass != nullptr && !Class->IsChildOf(Inner->PropertyClass))
		{
			OutError = FString::Printf(TEXT("%s is not a valid %s subclass"), *Class->GetPathName(),
									   *Inner->PropertyClass->GetName());
			return nullptr;
		}
		if (Class->HasAnyClassFlags(CLASS_Abstract))
		{
			OutError = FString::Printf(TEXT("Cannot instantiate abstract class %s; use a concrete Blueprint subclass"),
									   *Class->GetPathName());
			return nullptr;
		}
		...
		if (const FKDFNode* Properties = Entry.Find(TEXT("properties")))
		{
			for (const TSharedRef<FKDFNode>& PropertyRef : Properties->Sequence)
			{
				...
				if (!FKDFOpEngine::ParseOpEntry(PropertyRef.Get(), PathText, Op, Args, OutError) ||
					!FKDFPropertyPath::Parse(PathText, Path, OutError) ||
	...
	bool ApplyArrayOp(EKDFOp Op, UObject* Outer, const FArrayProperty* ArrayProperty, void* ValuePtr,
					  const FKDFOpArgs& Args, FString& OutError)
	{
		...
		case EKDFOp::Insert:
			{
				...
				const FObjectProperty* InstancedInner = GetInstancedInner(Inner);
				for (int32 Offset = 0; Offset < Elements.Num(); ++Offset)
				{
					if (InstancedInner != nullptr && Elements[Offset]->IsMap())
					{
						UObject* Instance =
							MakeInstancedObjectFromNode(Outer, InstancedInner, *Elements[Offset], OutError);
						if (Instance == nullptr)
						{
							return false;
						}
						InstancedInner->SetObjectPropertyValue(Helper.GetRawPtr(InsertAt + Offset), Instance);
						continue;
					}
	...
	{
		const FObjectProperty* Inner = GetInstancedInner(ArrayProperty->Inner);
		...
		for (const TSharedRef<FKDFNode>& EntryRef : Value.Sequence)
		{
			UObject* Instance = MakeInstancedObjectFromNode(Outer, Inner, EntryRef.Get(), OutError);
			if (Instance == nullptr)
			{
				return false;
			}
...
bool FKDFOpEngine::IsOpSupported(EKDFOp Op, const FProperty* LeafProperty)
{
	...
	case EKDFOp::Prepend:
		return LeafProperty->IsA<FArrayProperty>() || LeafProperty->IsA<FSetProperty>() ||
			...
			FKDFValueCodec::IsBitmaskProperty(LeafProperty);
	case EKDFOp::Insert:
	case EKDFOp::RemoveAt:
	case EKDFOp::Swap:
	case EKDFOp::Replace:
						   ...
						   FString& OutError)
{
	...
	if (FKDFValueCodec::IsBitmaskProperty(Resolved.mProperty))
	{
		return FinishWrite(ApplyBitmaskOp(Op, CastField<FNumericProperty>(Resolved.mProperty),
										 FKDFValueCodec::GetBitmaskEnum(Resolved.mProperty), Resolved.mValuePtr, Args,
										 OutError));
	}
	...
	if (const FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Resolved.mProperty))
	{
		return FinishWrite(ApplyArrayOp(Op, RootObject, ArrayProperty, Resolved.mValuePtr, Args, OutError));
	}
