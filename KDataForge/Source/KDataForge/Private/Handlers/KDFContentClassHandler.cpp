...
namespace
{
	...
	struct FKDFResearchNodeWork
	{
		...
		const FKDFNode* SourceNode = nullptr;
...
void UKDFContentClassHandler::CollectRemovals(const FKDFNode& Document, const FKDFContextBase& Context,
											  bool bReportDiagnostics, TArray<FKDFContentRemoval>& OutRemovals) const
{
	if (mRegistrationKind == EKDFContentRegKind::None)
	{
		return;
	}
	...
	if (!Entries->IsSequence() || Entries->Num() == 0)
	{
		if (bReportDiagnostics)
		{
			Context.AddError(TEXT("'remove' must be a non-empty sequence of class references"), Entries->Line);
		}
		return;
	}
	...
	FString BaseError;
	const UClass* RequiredBase = Subsystem->FindOrLoadClassCached(mRequiredBaseClassPath, BaseError);
	...
	for (const TSharedRef<FKDFNode>& EntryRef : Entries->Sequence)
	{
		...
		if (!Entry.IsScalar() || Entry.Scalar.IsEmpty())
		{
			if (bReportDiagnostics)
			{
				Context.AddError(TEXT("'remove' entries are bare class references, not maps"), Entry.Line);
			}
			continue;
		}
		...
		UClass* ContentClass = Subsystem->FindOrLoadClassCached(Entry.Scalar, Error);
		if (ContentClass == nullptr)
		{
			if (bReportDiagnostics)
			{
				Context.AddError(FString::Printf(TEXT("Could not resolve '%s': %s"), *Entry.Scalar, *Error),
								 Entry.Line);
			}
			continue;
		}
		if (RequiredBase != nullptr && !ContentClass->IsChildOf(RequiredBase))
		{
			if (bReportDiagnostics)
			{
				Context.AddError(
					FString::Printf(TEXT("'%s' is not a %s class"), *Entry.Scalar, *GetRootType().ToString()),
					Entry.Line);
			}
			continue;
		}

		FKDFContentRemoval& Removal = OutRemovals.AddDefaulted_GetRef();
		Removal.mContentClass = ContentClass;
		Removal.mKind = mRegistrationKind;
	}
}

bool UKDFContentClassHandler::ApplyRemovals(const FKDFNode& Document, FKDFApplyContext& Context)
{
	TArray<FKDFContentRemoval> Removals;
	CollectRemovals(Document, Context,  false, Removals);
	if (Removals.IsEmpty())
	{
		return false;
	}
	...
	for (FKDFContentRemoval& Removal : Removals)
	{
		Removal.mSourceFile = Context.mSourceFile;
		Removal.mPackRef = Context.mPackRef;

		Subsystem->RetainObject(Removal.mContentClass.Get());
		Subsystem->RegisterContentRemoval(Removal);
		...
		if (Context.mPatchRecord != nullptr)
		{
			...
			OpRecord.mTargetObjectPath = GetPathNameSafe(Removal.mContentClass.Get());
			OpRecord.mPropertyPath = TEXT("ContentRegistry");
			OpRecord.mOp = EKDFOp::Remove;
			OpRecord.mValueText = GetRootType().ToString();
		}
	}

	UE_LOG(LogKDataForge, Log, TEXT("Registered %d content removal rule(s) from %s"), Removals.Num(),
		   *Context.mSourceFile);
	return true;
}
...
bool UKDFContentClassHandler::ValidateDocument(const FKDFNode& Document, FKDFValidationContext& Context)
{
	...
	TArray<FKDFContentRemoval> Removals;
	CollectRemovals(Document, Context,  true, Removals);
	const bool bHasRemovals = !Removals.IsEmpty();
	...
	if (Entries == nullptr || !Entries->IsSequence() || Entries->Num() == 0)
	{
		if (!bHasRemovals)
		{
			Context.AddError(FString::Printf(TEXT("%s document requires a non-empty '%s' sequence or a 'remove' "
												  "sequence"),
														   ...
														   const TCHAR* ShortcutName, FKDFApplyContext& Context)
{
	...
	if (UnlockClass->HasAnyClassFlags(CLASS_Abstract))
	{
		Context.AddError(
			FString::Printf(TEXT("Cannot instantiate abstract shortcut unlock class %s"), *UnlockClass->GetPathName()),
			EntriesNode.Line);
		return false;
	}
																	...
																	FKDFApplyContext& Context)
{
	...
	Unlock->SetChild(TEXT("class"), FKDFNode::MakeScalar(FKDFBlueprintClassPath::UnlockScannableResource));
	Unlock->SetChild(TEXT("properties"), Properties);
	TSharedRef<FKDFNode> Unlocks = FKDFNode::MakeSequence();
	Unlocks->AddChild(Unlock);
	return FKDFInstancedObjectUtil::ApplyList(TargetCDO, Unlocks.Get(), TEXT("mUnlocks"),
										 ...
										 FKDFApplyContext& Context)
{
	...
	TMultiMap<FKDFGridPoint, FKDFGridPoint> AutoEdgesByParent;
	...
	if (bAutoPath)
	{
		for (FKDFResearchNodeWork& Work : WorkingNodes)
		{
			...
			{
				Roads.Add(
					TPair<FKDFGridPoint, TArray<FKDFGridPoint>>(Child, BuildAutoRoadPoints(Work.Coordinate, Child)));
			}
...
bool UKDFContentClassHandler::ApplyEntry(const FKDFNode& Entry, FKDFApplyContext& Context)
{
	...
	{
		const FString RequestedParentPath = Entry.GetString(TEXT("parent"), mDefaultParentPath);
		const FString ParentPath = FKDFBlueprintClassPath::Remap(RequestedParentPath);
	...
	if (bSupportsUnlocks)
	{
		...
		{
			FKDFInstancedObjectUtil::ApplyList(TargetCDO, *Unlocks, TEXT("mUnlocks"),
											   TEXT("Schematic class has no mUnlocks object array"), Context);
		}
		if (const FKDFNode* Recipes = Entry.Find(TEXT("recipes")))
		{
			ApplyClassListShortcutUnlock(TargetCDO, *Recipes, FKDFBlueprintClassPath::UnlockRecipe, TEXT("mRecipes"),
										 TEXT("/Script/FactoryGame.FGRecipe"), TEXT("recipes"), Context);
		}
		if (const FKDFNode* Schematics = Entry.Find(TEXT("schematics")))
		{
			ApplyClassListShortcutUnlock(TargetCDO, *Schematics, FKDFBlueprintClassPath::UnlockSchematic,
		...
		{
			ApplyCountShortcutUnlock(TargetCDO, *HandSlots, FKDFBlueprintClassPath::UnlockArmEquipmentSlot,
									 TEXT("mNumArmEquipmentSlotsToUnlock"), TEXT("handSlots"), Context);
		}
		if (const FKDFNode* InventorySlots = Entry.Find(TEXT("inventorySlots")))
		{
			ApplyCountShortcutUnlock(TargetCDO, *InventorySlots, FKDFBlueprintClassPath::UnlockInventorySlot,
	...
	if (bSupportsDependencies)
	{
		...
		{
			FKDFInstancedObjectUtil::ApplyList(TargetCDO, *Dependencies, TEXT("mSchematicDependencies"),
											   TEXT("Schematic class has no mSchematicDependencies object array"),
											   Context);
		}
	}
	if (bSupportsResearchDependencies)
	{
		if (const FKDFNode* Dependencies = Entry.Find(TEXT("unlockDependencies")))
		{
			FKDFInstancedObjectUtil::ApplyList(TargetCDO, *Dependencies, TEXT("mUnlockDependencies"),
											   TEXT("Research tree class has no mUnlockDependencies object array"),
											   Context);
		}
		if (const FKDFNode* Dependencies = Entry.Find(TEXT("visibilityDependencies")))
		{
			FKDFInstancedObjectUtil::ApplyList(TargetCDO, *Dependencies, TEXT("mVisibilityDependencies"),
											   TEXT("Research tree class has no mVisibilityDependencies object array"),
											   Context);
		}
	}
	if (bSupportsNodes)
...
bool UKDFContentClassHandler::ApplyDocument(const FKDFNode& Document, FKDFApplyContext& Context)
{
	...
	bool bAppliedAny = ApplyRemovals(Document, Context);
...
UKDFClassHandler::UKDFClassHandler()
{
	...
	mDefaultParentPath = FString();
	mRequiredBaseClassPath = FString();
...
UKDFResearchHandler::UKDFResearchHandler()
{
	...
	bSupportsNodes = true;
	bSupportsResearchDependencies = true;
}
