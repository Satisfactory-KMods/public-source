...
bool AKLMMBuildableMiner::CheckStorage() const
{
	...
	if (AKLMMBuildableModule* WasteModule = GetFirstModule(mWasteProducerAttachmentClass))
	{
		if (const TSubclassOf<UFGItemDescriptor> TrashItem = GetTrashDescriptor())
		{
			if (!WasteModule->CanStorageOutput(TrashItem, 1))
			{
				return false;
			}
		}
...
bool AKLMMBuildableMiner::CanRunModuleProduction() const
{
	AKLMMBuildableModule* WasteModule = GetFirstModule(mWasteProducerAttachmentClass);
	if (!WasteModule)
	{
		return true;
	}

	const TSubclassOf<UFGItemDescriptor> TrashItem = GetTrashDescriptor();
	if (!TrashItem)
	{
		return true;
	}

	return WasteModule->CanStorageOutput(TrashItem, GetCrusherProductionAmount());
}
