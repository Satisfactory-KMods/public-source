...
void UKPCLColoredStaticMesh::ApplyNewData()
{
	if (!mBlockInstancing)
	{
		if (mInstanceHandle.IsInstanced())
		{
			...
			if (Manager)
			{
				TArray<float> CustomData = mInstanceHandle.GetCustomData();
				CustomData.SetNum(mNumCustomDataFloats + mCustomExtraData.Num());
				mInstanceHandle.SetCustomDataArray(CustomData);
