...
void AKPCLNetworkCore::GetConditionalReplicatedProps(TArray<FFGCondReplicatedProperty>& outProps) const
{
	...
	FG_DOREPCONDITIONAL_WITH_NOTIFY(ThisClass, mMinimumStoragePercentage, OnRep_MinimumStoragePercentage);
}

void AKPCLNetworkCore::SetMinimumStoragePercentage(float NewPercentage)
{
	if (!HasAuthority())
	{
		if (UKPCLDefaultRCO* RCO = UKPCLDefaultRCO::Get(this))
		{
			RCO->Server_Delivery_SetMinimumStoragePercentage(this, NewPercentage);
		}
		return;
	}

	const float SafePercentage = FMath::IsFinite(NewPercentage) ? FMath::Clamp(NewPercentage, 0.f, 100.f) : 0.f;
	if (FMath::IsNearlyEqual(mMinimumStoragePercentage, SafePercentage))
	{
		return;
	}

	mMinimumStoragePercentage = SafePercentage;
	mPropertyReplicator.MarkPropertyDirty(FName("mMinimumStoragePercentage"));
	OnRep_MinimumStoragePercentage();
	ForceNetUpdate();
}

void AKPCLNetworkCore::OnRep_MinimumStoragePercentage()
{
	mMinimumStoragePercentage = FMath::Clamp(mMinimumStoragePercentage, 0.f, 100.f);
	mOnMinimumStoragePercentageChanged.Broadcast(mMinimumStoragePercentage);
}
...
int32 AKPCLNetworkCore::TryToGrabItemAmountAboveReserve(TSubclassOf<UFGItemDescriptor> Item, int32 Amount)
{
	if (!HasAuthority() || !IsValid(Item) || Amount <= 0)
	{
		return 0;
	}

	const float SafePercentage = GetMinimumStoragePercentage();
	const int32 MinimumRemaining =
		FMath::CeilToInt32(static_cast<double>(GetMaxItemAmount(Item)) * static_cast<double>(SafePercentage) / 100.0);
	...
	int32 RemovedAmount = 0;
	{
		...
		FScopeLock ScopeLock(&mMutex);
		bool bHasItem = false;
		const FItemAmount CurrentAmount = mStorage.GetAmountConst(Item, bHasItem);
		const int32 RemovableAmount = bHasItem ? FMath::Max(0, CurrentAmount.Amount - MinimumRemaining) : 0;
		RemovedAmount = mStorage.RemoveItemAmount(Item, FMath::Min(Amount, RemovableAmount));
	}
	...
	if (RemovedAmount > 0)
	{
		...
		{
			RunOnGameThreadIfValid(this, [](AKPCLNetworkCore* Self) { Self->NotifyStorageChange(); });
		}
	}
