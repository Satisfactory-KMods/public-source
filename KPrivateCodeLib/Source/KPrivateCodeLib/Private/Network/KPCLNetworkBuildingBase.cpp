...
FKPCLFaxitNetwork AKPCLNetworkBuildingBase::GetNetworkData_Implementation() const
{
	...
	AKPCLFaxitSubsystem* FaxitSubsystem = GetFaxitSubsystem();
	if (!IsValid(FaxitSubsystem))
	{
		return Outer;
	}

	FaxitSubsystem->GetNetwork(this, Outer);
	return Outer;
}
...
FKPCLFaxitNetwork AKPCLNetworkBuildingBase::GetFaxitNetwork() const
{
	...
	bool Success;
	return FaxitSubsystem->GetByNetworkId(GetNetworkId(), Success);
}
...
TArray<EKPCLNetworkError> AKPCLNetworkBuildingBase::GetNetworkErrorCodes() const
{
	...
	bool Success;
	FKPCLFaxitNetwork Network = FaxitSubsystem->GetByNetworkId(GetNetworkId(), Success);
	...
	if (FaxitSubsystem->HasToManyNetworks())
	{
...
AKPCLFaxitSubsystem* AKPCLNetworkBuildingBase::GetFaxitSubsystem() const
{
	if (IsValid(mFaxitSubsystem))
	{
		return mFaxitSubsystem;
	}

	return AKPCLFaxitSubsystem::Get(GetWorld());
}
