...
UCLASS()
class KPRIVATECODELIB_API UKPCLBaseClipboardSettings : public UFGFactoryClipboardSettings
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(BlueprintReadWrite)
	float mTargetPotential = 1.f;

	UPROPERTY(BlueprintReadWrite)
	TArray<FItemAmount> mShards;
};

USTRUCT(BlueprintType)
struct FKPCLMeshOverwriteInformation
{
	GENERATED_BODY()

	UPROPERTY(SaveGame, BlueprintReadOnly, EditAnywhere)
	int32 mOverwriteHandleIndex = -1;

	UPROPERTY(SaveGame, BlueprintReadOnly, EditAnywhere)
	UStaticMesh* mOverwriteMesh = nullptr;

	UPROPERTY(SaveGame, BlueprintReadOnly, EditAnywhere)
	TMap<int32, UMaterialInterface*> mMaterialOverwriteMap;

	UPROPERTY(SaveGame, BlueprintReadOnly, EditAnywhere)
	bool mUseCustomTransform = false;

	UPROPERTY(SaveGame, BlueprintReadOnly, EditAnywhere)
	FTransform mCustomTransform;
};
...
UENUM()
enum class EKPCLConnectionType : uint8
{
	ConvIn = 0,
	ConvOut = 1,
	PipeIn = 2,
 ...
 */
UCLASS(Abstract)
class KPRIVATECODELIB_API AKPCLProducerBase : public AFGBuildableFactory, public IFGRecipeProducerInterface,
...
public:
	...
	bool bEnableCustomOverclocking = true;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "KMods ")
	static int32 GetFullestStackIndex(UFGInventoryComponent* InventoryComponent);
