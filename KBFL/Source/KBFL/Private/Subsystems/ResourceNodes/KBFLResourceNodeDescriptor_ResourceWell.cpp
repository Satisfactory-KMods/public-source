			...
			for (TTuple<TEnumAsByte<EResourcePurity>, FKBFLTransformArray> PurityMap : WellData.mPurityLocations)
			{
				...
				for (FTransform Location : PurityMap.Value.mTransforms)
				{
					if (Location.GetLocation() != FVector(0))
					{
						...
						AActor* SatelliteActor = nullptr;
						if (!CheckActorInRange(Location, SatelliteActor) && IsRangeFree(Location))
						{
							SatelliteActor = SpawnActorAtLocation(Location, mFrackingSatelliteClass);
						}

						if (SatelliteActor)
						{
							ApplyMaterialData(SatelliteActor, WellData.mFrackingSatelliteMaterialInfo);
							ActorArray.Add(SatelliteActor);
						}

						if (AFGResourceNodeFrackingSatellite* Satellite =
								Cast<AFGResourceNodeFrackingSatellite>(SatelliteActor))
						{
